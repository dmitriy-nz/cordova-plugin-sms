...
public class SMSPlugin
  ...
  private static final String ACTION_READ_SMS = "readSMS";
  ...
  public boolean execute(String action, JSONArray inputs, CallbackContext callbackContext) throws JSONException {
    ...
    } else if (ACTION_READ_SMS.equals(action)) {
      JSONObject params = inputs.optJSONObject(0);
      this.markSmsAsRead(params, callbackContext);
    } else {
  ...
  private void markSmsAsRead(JSONObject params, CallbackContext callbackContext) {
    Activity context = this.cordova.getActivity();

    try {
      final String address = params.optString("address");
      Log.d(LOGTAG, "markSmsAsRead: address " + address);

      Uri uri = Uri.parse(SMS_URI_INBOX);
      String where = "address = ? AND read = ?";
      String[] selectionArgs = {address, "0"};

      ContentValues values = new ContentValues();
      values.put("read", true);


      int readTotal = context.getContentResolver().update(uri, values, where, selectionArgs);

      if (callbackContext != null)
        callbackContext.success(readTotal);
